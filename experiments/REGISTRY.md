...
**Verdict, committed (no hedging):** v1's p=0.013 was **optimistic** — inflated by persistent-winner concepts shared across its temporal select/test. Under a concept-disjoint split (the honest test), **the detector's edge is NOT established.** The project should not claim a "proven detector edge." What *is* proven is the **method**: a pre-registered, contamination-free apparatus that caught its own false positive.

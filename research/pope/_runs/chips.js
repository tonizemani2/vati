export const meta = {
  name: 'pope-pro-chips',
  description: 'Pope PRO, domain BAKED IN (semiconductors/chips). Opus ideation+synthesis, Sonnet gate. Domain hardcoded because the launcher drops args; channels trimmed to 5.',
  phases: [
    { title: 'Generate', detail: 'Opus channel miners (chips)', model: 'opus' },
...
const domain = 'semiconductors and the chip supply chain (logic foundry, memory and HBM, advanced packaging and substrates, lithography and fab equipment and specialty gases/materials, EDA and IP, test, and the compute hardware stack)'
const GEN_MODEL = 'opus', GATE_MODEL = 'sonnet', SYNTH_MODEL = 'opus'
const perChannel = 2, topK = 6, date = '2026-06-14'

const ALL_CHANNELS = [
  { key: 'physical-limits', lens: 'a hard physical or thermodynamic limit (heat, power density, atomic-scale variability, lithography wavelength, defectivity) that forces a shift almost nobody is pricing' },
  { key: 'materials-chokepoint', lens: 'an inelastic upstream material, specialty gas, or midstream processing chokepoint hidden beneath a popular theme; a granular sub-node nobody stockpiled' },
  { key: 'constraint-migration', lens: 'a constraint-migration cascade: once the obvious bottleneck (HBM, CoWoS) gets funded, rent jumps one layer upstream to an unpriced node' },
  { key: 'pricing-arbitrage', lens: 'something structurally true and near-certain markets have not priced because it is boring, invisible, or hard to financialize (trained process engineers, fab construction trades, qualification time)' },
  { key: 'second-order', lens: 'the second-order consequence the obvious trend (AI compute demand) forces next, which the loud first-order narrative ignores' },
  { key: 'wildcard', lens: 'a deliberately contrarian, anti-consensus, maximally disruptive chips call; aperture fully open (the gate keeps it honest later)' },
]
...
GROUNDING: first Read FUTURE_MAP.md in the repo root and skim existing calls in or near this area (it has a deep compute-frontier channel: HBM, logic base die, substrates, photonics, packaging), so you do NOT duplicate them. Go deeper or adjacent.

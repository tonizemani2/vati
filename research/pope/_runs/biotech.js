export const meta = {
  name: 'pope-pro-biotech',
  description: 'Pope PRO, domain BAKED IN (biotech/health). Opus ideation+synthesis, Sonnet gate. Domain hardcoded because the launcher drops args; channels trimmed to 5.',
  phases: [
    { title: 'Generate', detail: 'Opus channel miners (biotech/health)', model: 'opus' },
...
const domain = 'biotech and human health (drug modalities and delivery, biomanufacturing and fill-finish capacity, donor-derived and cell/gene inputs, diagnostics and tools/reagents, medical isotopes and devices, longevity and aging)'
const GEN_MODEL = 'opus', GATE_MODEL = 'sonnet', SYNTH_MODEL = 'opus'
const perChannel = 2, topK = 6, date = '2026-06-14'

const ALL_CHANNELS = [
  { key: 'physical-limits', lens: 'a hard physical or biological limit (delivery across a tissue barrier, thermodynamics/throughput of manufacturing, a donor-rate ceiling) that forces a shift almost nobody is pricing' },
  { key: 'materials-chokepoint', lens: 'an inelastic upstream reagent, donor-derived biologic, isotope, or single-vendor input/consumable hidden beneath a popular modality' },
  { key: 'constraint-migration', lens: 'a constraint-migration cascade: once the headline modality (editor, antibody, GLP-1, cell therapy) is funded, rent jumps one layer upstream to delivery, fill-finish, or a shared input nobody prices' },
  { key: 'pricing-arbitrage', lens: 'something structurally true and near-certain markets have not priced because it is boring or hard to financialize (GMP manufacturing slots, qualified process engineers, donor pools, cold-chain, sterile capacity)' },
  { key: 'second-order', lens: 'the second-order consequence the obvious trend (GLP-1 mass adoption, in-vivo editing, aging demographics) forces next, which the loud first-order narrative ignores' },
  { key: 'wildcard', lens: 'a deliberately contrarian, anti-consensus, maximally disruptive biotech/health call; aperture fully open (the gate keeps it honest later)' },
]
...
const STYLE = 'Write prose in plain, human English. No em-dashes. No promotional filler. Physical/biological mechanism first. Name the inelastic needle, never a vague theme.'
...
GROUNDING: first Read FUTURE_MAP.md in the repo root and skim existing calls in or near this area (it has a biotech-health channel: delivery, AAV capacity, plasmid/DNA, peptide CDMOs, ADC conjugation), so you do NOT duplicate them. Go deeper or adjacent.

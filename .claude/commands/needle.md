...
4. **Decompose theme → constraint (in-session — the generative act).** Ground it in the MEASURED dependency graph first: `signals "<survivor>"` now surfaces a **DEPENDENCY GRAPH** block from the OpenAlex citation flow — `draws_on` (what the concept's knowledge base leans on → candidate binding inputs) and `drawn_on_by` (who leans on it → blast radius if it is constrained). Walk those measured edges (don't assert links), then through a Bucket-2 lens (toc / ricardian / helmer / inversion / analogy) name the **specific** inelastic input — a named consumable / material / isotope / sub-component, with **named suppliers** and a multi-year expansion lead time. ("AI" → not "compute" but e.g. "ABF substrate, Ajinomoto >95%"). The top outbound edge is often a hypernym; the needle is a **mid-weight** substantive input or a **high inbound load**. 2–4 candidates per theme.
5. **Run the consensus eye at structural altitude.** `consensus-eye "<the structural claim>"` — narrative saturation + the **consensus-forecast channel** (have IEA/IMF/banks already projected it? → priced) + an optional price run-up if a clean ticker exists. Then `market-anchor "<the claim>"` — is a LIVE prediction market already trading it (Manifold/Metaculus, keyless)? A liquid match = PRICED; the edge is only the GAP to your P. Edge lives ONLY where all channels are quiet. A claim with no instrument/market is still valid — say so; `UNPRICED-UNSEEN` is not a green light.
